...
getting attention from expert mentors.  It will work toward a truly
scalable assessment system---one that can serve as an educational
platform for the many millions of people around the world who could
benefit from a high quality mathematics education. 
...
practice before they become reasonably proficient at grading proofs
from a rubric.  As such, it cannot serve as
...
\end{center}
\caption{User Interface for Selecting Attributes (which were called ``judgments'' in the original system)}
...
conducted in one of the classes.  The system worked better than existing
...
review the proofs themselves.  The instructors found the
...
students in large-scale learning environments
to learn how to write and evaluate proofs.  We propose
...
as can be captured by a proof graph \citep{anderson-science85}.
...
weights when randomly assigning proofs to reviewers.
...
proofs that are known to contain specific flaws, so that students get
practice identifying these classes of errors.  Similar strategies
...
As we have discussed, a web-scale course cannot rely on a
...
We cannot have paid staff directly oversee the efforts of
individual students, as we did with large classes.  We must 
...
testing.  We can also conduct experiments at very fine levels
...
as how proofs are assigned to reviewers, what frameworks are provided
